...
const Classifiers = [
  ...
  { type: Directive.Requirement, matcher: /^requirement "(.+)","([^"]*)"$/ },
]

const trim = x => x.trim()
const notEmpty = x => x.length > 0

module.exports = function parseDataFile({ file, warn = false }) {
  ...
  return lines.reduce(function(list, line, index) {
    ...
    switch (directive.type) {
      ...
      case Directive.Requirement:
        list.push({
          Id: directive.data[0],
          Constraints: directive.data[1],
        })
